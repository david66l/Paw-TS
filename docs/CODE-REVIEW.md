...
| 24 | 🟡 `TURN_FLAG_CODECS` 登记表已落地并**被编译器强制**（新增 `TurnFlags` 字段不登记就编译失败，已 A/B 验证），`restore` 的返回类型改为由它推导、22 名字的 `Pick` 清单删除。**剩余部分本轮查清了性质**：手工 spread（`:196-282`）无法从登记表机械推导，因为登记表只有"属于哪个 section"，没有"每条 flag 自己的省略规则"；而一致性用例只手写了 7/22 条 flag。两个剩余项**收敛到同一个前置条件**：把登记表扩展成 `{section, omitRule\|sample}`（见 11.37） | 消掉"加字段就静默丢状态"的悬崖 | 中 |
...
| 29 | ✅ 完成：`toWorkspaceRelPath()` ✅、共享 `summarizeToolArgs` ✅、**两处永不执行的收尾冲刷已删除**（99 行，判定依据是穷举循环出口，见 11.39）、**`sse.ts` 已抽出**（`takeSseDataPayloads`，两处读循环收敛，两个调用点的载荷解析与发射路径一行未动，7 个用例钉行处理边界，见 11.40）| 消掉已分叉副本 | 中 |
...
### 11.37 #24 剩余部分：两件看起来不同的事，其实卡在同一个前置条件上

#24 的行总结说"`TURN_FLAG_CODECS` 登记表已落地并被编译器强制…编码/解码函数体仍是手工 spread"。本轮把剩余部分查清楚了，结论是**它不能靠"再写个测试"或"再抽个函数"收尾**。

**一、手工 spread 无法从登记表机械推导。** `TURN_FLAG_CODECS`（`loop-control-state.ts:85-111`）登记的是**每条 flag 属于哪个 section**（`providerTerminal` / `readiness` / `protocolRecovery` / `completionGates` / `lateGuidance` / `toolLoop`，或 `null` 豁免）。而编码器（`:196-282`，约 90 行）逐条写的是**每条 flag 自己的省略规则**：`> 0` 才写（计数器）、真值才写（布尔）、要深拷贝（`[...flags.failureSignatures]`）、改名（`_maxStepsWarned` → `maxStepsDelivered`）。这些规则本来就是**逐字段不同**的，登记表里没有它们，所以"从登记表生成编码器"这件事缺的不是代码，是**信息**。

**二、登记表强制的是"完整性"，而那正是 #24 当初要的东西。** 编译器保证的是：`TurnFlags` 新增字段若不登记就编译失败（`as const satisfies { readonly [K in keyof TurnFlags]-?: CheckpointSectionV1 | null }`，已在早期轮次 A/B 验证）。也就是"加字段就静默丢状态"这个悬崖已经拆掉了。

**三、真正剩下的缺口，是登记表覆盖不到的**：`loop-control-state.test.ts:43` 那条"登记表与编码器在 section 上一致"的用例，用的是**手写的 7 个 flag 清单**（`:55-65` 与 `:74-84`），而登记表里有 **22 条**非 null 的 flag。也就是说**有 15 条已登记的 flag 没有被这条一致性用例覆盖** —— 如果某条 flag 登记进了一个**已经存在**的 section，而编码器/解码器两边都没处理它，这条用例不会红，编译器也不会红（`restore` 的返回类型是 `Partial<...>`，本来就允许省略）。

**四、于是两个剩余项收敛到同一个前置条件**：要泛化地堵住上面那个缺口，测试需要为**每条**已登记 flag 构造一个"非默认值"以迫使它出现在产物里 —— 而那正是"每条 flag 的省略规则"这份信息，也就是第一点里缺的东西。**所以正确的下一步是把登记表从 `section` 扩展成 `{ section, omitRule | sample }`**，而不是继续手写清单或继续手工 spread。这一步之后，编码器可以部分生成、一致性测试也能变成从登记表派生的循环。

**本轮没有改代码**：这是把 #24 的剩余面从"看起来是收尾工作"改判为"需要一次登记表结构扩展"的设计任务，避免后续轮次再去尝试那个做不到的机械推导。

### 11.38 #29 剩余的 `sse.ts`：抽取面比想象小，但先要确认一件事

第 54 轮我把 #29 的剩余部分判为"纯可读性、风险不划算"，那判断**下得太粗**。本轮把四处调用点读了一遍，实际形状是：

同一段"按 `\n` 切分 → 不完整的尾巴留回缓冲 → 逐行去 `\r` 与空白 → 取 `data: ` 之后的载荷"，在 **2 个文件里各出现 2 次**：

| 文件 | 读循环内 | 收尾冲刷残留缓冲 |
|---|---|---|
| `openai-compatible.ts` | `:434-444` | `:518` 附近 |
| `anthropic-compatible.ts` | `:427-439` | `:444-453` |

**好消息是抽取面很小**：需要抽的只是**纯字符串处理**（`(buffer, done) → { payloads, carry }`），两个调用点各自的载荷解析（`parseOpenAiChatCompletionStreamDataPayload` / `parseAnthropicStreamPayload`）与发射路径（`emitModelObservation` / `yield chunk`）**都不用动**。也就是说这次抽取**不触碰流式状态机** —— 我上一轮担心的"要重接两个不同的状态机"并不成立，那是我没读代码就下的结论。

**但先要确认一件事，否则抽出来的会包含一段死代码**：`anthropic-compatible.ts:427-428` 在进循环体之前就把 `buffer` 写成了 `done ? "" : (lines.pop() ?? "")` —— 也就是说 `done` 为真时 `buffer` 已是空串，于是循环之后 `:444` 那句 `if (buffer.trim())` 的收尾冲刷**永远不成立**。

- 若确为死代码 → 抽取时**只抽读循环那一处**，收尾冲刷直接删掉（连同 `openai-compatible.ts` 的对应处），并且这本身就是一处值得单独记的发现：一段"以防万一"的兜底代码因为上游赋值而永不执行。
...
### 11.40 #29 完成：`sse.ts` 抽出，两处读循环收敛

新增 `packages/models/src/sse.ts`，导出 `takeSseDataPayloads(buffer, done): { payloads, carry }` —— 就是上一条记下的纯字符串处理。两个调用点（`openai-compatible.ts:434`、`anthropic-compatible.ts:427`）改为：

```ts
buffer += decoder.decode(value ?? new Uint8Array(), { stream: !done });
const { payloads, carry } = takeSseDataPayloads(buffer, done);
buffer = carry;
for (const payload of payloads) { /* 各自的解析与发射，未改动 */ }
```

**两个调用点各自的载荷解析（`parseOpenAiChatCompletionStreamDataPayload` / `parseAnthropicStreamPayload`）与发射路径（`emitModelObservation` / `yield chunk`）一行未动**，所以这次抽取不触碰流式状态机 —— 这也正是它能在不改行为的前提下完成的原因。`openai` 侧的 `sawDoneMarker` 守卫在循环体内、`data:` 过滤之后，位置保持原样。
