...
export type CompletionReviewEvidenceOutcomeV1 = "passed" | "failed" | "indeterminate";

export type CompletionReviewVerificationKindV1 = "test" | "build" | "lint" | "typecheck" | "none";
...
export function createCompletionReviewCandidateV1(
  ...
  if (!Number.isSafeInteger(input.sourceThroughSeq) || input.sourceThroughSeq <= 0) {
  ...
  const toolEvidence = Object.freeze(input.toolEvidence.map((item) => freezeEvidence(item)));
  if (input.environmentRevision !== undefined && !/^[a-f0-9]{64}$/.test(input.environmentRevision))
    throw new Error("Invalid environment revision");
  const value = Object.freeze({
    ...(input.environmentRevision ? { environmentRevision: input.environmentRevision } : {}),
  ...
  const candidateHash = createHash("sha256").update(JSON.stringify(value)).digest("hex");
  return Object.freeze({ candidateHash, ...value });
}

function freezeEvidence(value: CompletionReviewToolEvidenceV1): CompletionReviewToolEvidenceV1 {
  if (!value.callId.trim() || !value.tool.trim() || !value.summary.trim()) {
    throw new Error("Completion review tool evidence is invalid");
  }
  if (
    !["completed", "failed", "cancelled", "unknown", "rejected"].includes(value.executionStatus)
  ) {
    throw new Error("Completion review tool status is invalid");
  }
  if (!(["passed", "failed", "indeterminate"] as const).includes(value.outcome)) {
    throw new Error("Completion review tool outcome is invalid");
  }
  if (!(["test", "build", "lint", "typecheck", "none"] as const).includes(value.verificationKind)) {
    throw new Error("Completion review verification kind is invalid");
  }
  if (value.verificationTarget !== undefined && !value.verificationTarget.trim()) {
    ...
    ...(value.observedOutput
      ? {
          observedOutput: Object.freeze({
            ...
            truncated: value.observedOutput.truncated || value.observedOutput.text.length > 4_000,
          }),
        }
      : {}),
