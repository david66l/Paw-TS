...
describe("Paw Next frozen MCP proxy plugin", () => {
  test("keeps the provider schema stable while scope changes registry identity", () => {
    ...
    expect(firstPlugin.entries[0]?.definition).toEqual(secondPlugin.entries[0]?.definition);
    ...
    expect(first.resolveProviderName("workspace_use_mcp")?.internalName).toBe(MCP_PROXY);
  });
  ...
  test("rejects malformed scope identity and invalid proxy arguments", () => {
    ...
    const invalid = registry.validateAndClassify(call("invalid", { action: "other" }), os.tmpdir());
...
function call(id: string, args: Readonly<Record<string, unknown>>): RuntimeToolCallV1 {
  return {
    id,
