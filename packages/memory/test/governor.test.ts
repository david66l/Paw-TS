...
describe("时序倒挂规则（§7.4）", () => {
  ...
  test("候选更新 / 相似已失效 → 不倒挂", () => {
    ...
    const dead = makeFact("dead", {
      tValid: "2026-08-01T00:00:00Z",
      tInvalid: "2026-08-03T00:00:00Z",
    });
...
describe("parseGovernorOutput 校验", () => {
  test("合法输出映射序号→裁决", () => {
      ...
      JSON.stringify({
        decisions: [
          { candidate: 1, op: "ADD", target: null, reason: "new" },
          {
            ...
            op: "INVALIDATE",
            target: 1,
            reason: 'candidate 说"用 vitest" vs E-1 说"用 jest"',
          },
        ],
      }),
      2,
    ...
    expect(r.byCandidate.get(0)).toEqual({
      op: "ADD",
      targetSeq: undefined,
      reason: "new",
    });
  ...
  test("UPDATE/INVALIDATE 缺 target → error", () => {
    const r = parseGovernorOutput(
      JSON.stringify({
        decisions: [{ candidate: 1, op: "UPDATE", target: null }],
      }),
      1,
      1,
    );
...
describe("buildAdjudicationPrompt", () => {
  test("整数序号 + 纪律文本", () => {
    const prompt = buildAdjudicationPrompt([
      {
        candidate: makeFact("fact one"),
        similar: [makeFact("existing one", { id: "semantic-abc" })],
      },
...
describe("Governor db 集成", () => {
  ...
  it("§5.8-4 矛盾事实（jest→vitest）→ 旧条目失效 + 新条目 ADD + 检索只返回新条目 + why 可查历史", async () => {
    // 既有条目：jest
    const old = makeFact("The project uses jest for unit testing", {
      repo: REPO,
    });
    ...
    // 检索只返回新条目
    const recalled = await hybridRecall(engine, "unit testing framework", {
      candidates: 10,
    });
  ...
  it("UPDATE → history 追加旧值 + 账本保留", async () => {
    const old = makeFact("Tests run with a custom runner before each commit", {
      repo: REPO,
    });
    await engine.put(old);
    const oldId = deriveEntryId(old);
  ...
  it("批量裁决：3 条候选一次 LLM 调用返回混合裁决", async () => {
        ...
        return JSON.stringify({
          decisions: [
            ...
            { candidate: 2, op: "NOOP", target: null, reason: "equivalent" },
            {
              ...
              op: "INVALIDATE",
              target: m ? Number(m[1]) : 1,
              reason: 'candidate 说"cache disabled" vs E 说"cache enabled"',
            },
          ],
        });
      },
    ...
    const candidates = [
      {
        kind: "semantic",
        fact: "Lints run automatically on every save operation",
        keywords: [],
        evidence: [`runs/${RUN}#s1`],
      },
      {
        kind: "semantic",
        fact: "Builds use the custom bundler with cache enabled",
        keywords: [],
        evidence: [`runs/${RUN}#s2`],
      },
      {
        kind: "semantic",
        fact: "Builds use the custom bundler with cache disabled after migration",
        keywords: [],
        evidence: [`runs/${RUN}#s3`],
      },
    ];
    const p = new MemoryWritePipeline({
      distiller: distillerWith(candidates),
      governorLlm: govLlm,
    });
    const r = await p.processEvent({
      type: "task_succeeded",
      runId: `${RUN}_batch`,
      trajectoryRef: `runs/${RUN}_batch`,
      repo: REPO,
      goal: "g",
      trajectory: "t",
      verdict: { kind: "test", passed: true },
  ...
  it("时序倒挂：候选 tValid 早于既有条目 → 不进 LLM 直接 NOOP", async () => {
    const existing = makeFact("The project uses vitest for unit testing", {
      repo: REPO,
    ...
    const p = new MemoryWritePipeline({
      distiller: distillerWith(staleCandidate),
      governorLlm: govLlm,
    });
    const r = await p.processEvent({
      type: "task_succeeded",
      runId: `${RUN}_stale`,
      trajectoryRef: `runs/${RUN}_stale`,
      repo: REPO,
      goal: "g",
      trajectory: "t",
      verdict: { kind: "test", passed: true },
    ...
    expect(entries.map((e) => (e as SemanticFact).fact)).not.toContain(
      "The project used jest for unit testing last year",
    );
  });
  ...
  it("幻觉序号：LLM 返回越界序号 → 该条 NOOP + op-log 记录", async () => {
    ...
    const c1 = makeFact("Governor hallucination probe fact one", {
      repo: REPO,
    });
    const c2 = makeFact("Governor hallucination probe fact two", {
      repo: REPO,
    });
