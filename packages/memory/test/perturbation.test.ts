...
describe("summarizeCounterfactual", () => {
  ...
  test("纠正率 ≥80% 达标；未召回计入召回率不计入纠正率分母", () => {
    ...
    const ok = summarizeCounterfactual([
      mk("a", true, "corrected"),
      mk("b", true, "corrected"),
      mk("c", true, "corrected"),
      mk("d", true, "corrected"),
...
describe("summarizeNegation", () => {
  ...
  test("保持率 / 注入保真率 / 100% 达标", () => {
    const all = [
      mk("a", true, true, "kept"),
      mk("b", true, false, "kept"),
      mk("c", true, true, "reversed"),
    ];
...
describe("renderRedteamReport", () => {
  ...
  test("样本不足与预算截断提示", () => {
    const t = renderRedteamReport({
      ...base,
      passed: null,
      efficiency: { ...base.efficiency, truncated: true },
    });
...
describe("resolveLlmConfig", () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "paw-llmcfg-"));
  });
  ...
  test("未知 provider / 完全无配置 → 报错（不抛异常）", () => {
    writeSettings(
      JSON.stringify({
        provider: "default-p",
        models: { flash: { baseUrl: "https://b", model: "m2" } },
      }),
    );
...
describe("findSettingsFile", () => {
  test("从 startDir 向上找 ≤4 层；超过返回 null", () => {
    ...
    try {
      ...
      expect(findSettingsFile(join(dir, "a", "b", "c"), 4)).toBe(
