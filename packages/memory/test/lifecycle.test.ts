...
describe("isAutoMode（灰度判定）", () => {
  ...
  test("CLI 解析 gc 参数", () => {
    ...
    expect(parseMemoryArgs(["gc", "--review"])).toEqual({
      subcommand: "gc",
      review: true,
    });
    expect(parseMemoryArgs(["gc", "--approve", "semantic-x"])).toEqual({
      subcommand: "gc",
      approve: "semantic-x",
    });
    expect(parseMemoryArgs(["gc", "--sweep"])).toEqual({
      subcommand: "gc",
      sweep: true,
    });
  });
...
describe("生命周期 db 集成（§7.9）", () => {
  ...
  it("§7.9-1/2 效用删除候选判定：8 任务全失败进候选，5 任务/freq=2 受保护", async () => {
    ...
    const eight = await putTracked(
      makeSemantic("Amber entry with eight failed task participations"),
    );
    for (let i = 0; i < 8; i++)
      await recordRetrievalHits(engine, [eight], { runId: `${RUN}_8_${i}` });
  ...
  it("§7.9-3 user_statement 永不进候选；profile supportCount≥3 豁免", async () => {
    const user = await putTracked(
      makeSemantic("用户说永远先跑类型检查", {
        source: "user_statement",
        confidence: 1.0,
      }),
    );
    for (let i = 0; i < 10; i++)
      await recordRetrievalHits(engine, [user], { runId: `${RUN}_u_${i}` });
    ...
    const profileStrong: ProfileInsight = {
      ...
      freq: 0,
      utility: 0,
      insight: "Runs full test suite before every commit",
      supportCount: 3,
    };
    const strongId = await putTracked(profileStrong);
    for (let i = 0; i < 10; i++)
      await recordRetrievalHits(engine, [strongId], { runId: `${RUN}_p_${i}` });

    const profileWeak: ProfileInsight = {
      ...profileStrong,
      insight: "Sometimes checks the changelog",
      supportCount: 2,
    };
    const weakId = await putTracked(profileWeak);
    for (let i = 0; i < 10; i++)
      await recordRetrievalHits(engine, [weakId], { runId: `${RUN}_w_${i}` });
  ...
  it("CLI gc --review / --approve 可走通", async () => {
    const viaCli = await putTracked(
      makeSemantic("Flint entry approved through the CLI review path"),
    );
    for (let i = 0; i < 8; i++)
      await recordRetrievalHits(engine, [viaCli], { runId: `${RUN}_c_${i}` });
    await runLifecycleOnce({ config: { repo: REPO } });
  ...
  it("trial 容量：超 50 FIFO 丢弃 + attemptsLeft 耗尽物理删", async () => {
    ...
    const [cnt] =
      await sql`SELECT count(*)::int AS n FROM memory_trial_lessons WHERE origin_task_id = ${RUN + "_origin"}`;
