...
describe(".gitignore 默认项（§4.1）", () => {
  function checkIgnore(path: string): { ignored: boolean; rule: string } {
    const r = Bun.spawnSync(["git", "check-ignore", "-v", path], {
      cwd: REPO_ROOT,
    });
...
describe("memory-config.json", () => {
  test("缺文件 → 默认配置；写入 → 读回", async () => {
    ...
    try {
      ...
      expect(await loadMemoryConfig(root)).toEqual({
        enable: true,
        readonly: true,
        shadow: false,
      });
...
describe("export + readonly db 集成", () => {
  ...
  it("export：密钥条目跳过、高熵打码、报告正确", async () => {
    ...
    const withEntropy = makeSemantic(
      "migration marker x9Qv2mZ8kLp4Wn7Bz3Ya6TqR appears in legacy rows",
    );
  ...
  it("readonly：enqueue 丢弃 + op-log 记录，检索正常", async () => {
    ...
    const pkg = await new TriggeredRetriever({
      ...
      type: "explicit_query",
      question: "gypsum readonly probe",
      repo: REPO,
      runId: `${RUN}_ro_q`,
    });
