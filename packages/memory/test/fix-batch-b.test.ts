...
describe("parseTrialLessonOutput（#7）", () => {
  test("合法教训通过", () => {
    const r = parseTrialLessonOutput(
      JSON.stringify({
...
describe("parseQueryRewrite（#9）", () => {
  test("提炼词通过；JSON 残骸/超长/空 → null", () => {
    expect(parseQueryRewrite("quartz deadlock shutdown hooks")).toBe(
      "quartz deadlock shutdown hooks",
    );
...
describe("修复批次 B db 集成", () => {
  ...
  it("#6 episodic 蒸馏候选正常落库走 Governor；不支持 kind 记 op-log", async () => {
    const govLlm: GovernorLlm = {
      complete: async () =>
        JSON.stringify({
          decisions: [{ candidate: 1, op: "ADD", target: null, reason: "new" }],
        }),
    };
          ...
          candidates: [
            {
              ...
              failureFixPair: {
                failed: "deploy",
                feedback: "CredentialMismatchError",
                fixed: "rotate window",
              },
  ...
  it("#7 trial 教训由 LLM 蒸馏（含检索键）；超预算降级原文切片标注", async () => {
    ...
    const p = new MemoryWritePipeline({
      distiller: new MemoryDistiller(trialLlm),
    });
    const r = await p.processEvent({
      type: "task_failed",
      runId: `${RUN}_trial`,
      trajectoryRef: `runs/${RUN}_trial`,
      repo: REPO,
      goal: "rotate garnet token",
      trajectory: "rotated token directly, deploy failed",
    });
    ...
    const r2 = await p2.processEvent({
      type: "task_failed",
      runId: `${RUN}_trial2`,
      trajectoryRef: `runs/${RUN}_trial2`,
      repo: REPO,
      goal: "g2",
      trajectory: "raw failure slice content",
    });
  ...
  it("#8 trial 随行注入 attemptsLeft 递减，3 次后被 janitor 丢弃", async () => {
    ...
    const filler = makeSemantic(
      "OlivineModuleResolutionError handling requires a config reload first",
    );
  ...
  it("#9 T1 query 改写：提炼词与原文双路召回都被使用", async () => {
    ...
    const rewriter = {
      complete: async () => "sphene liveness probes flap rolling updates",
    };
  ...
  it("#9 query 改写失败 → 降级原文单路 + degraded", async () => {
    ...
    const r = new TriggeredRetriever({
      engine,
      queryRewriter: badRewriter,
      countTokens: (t) => Math.ceil(t.length / 4),
    });
    const runId = `${RUN}_qr_fail`;
    const pkg = await r.retrieve({
      type: "task_start",
      taskDescription: "sphene rewrite fallback probe",
      repo: REPO,
      runId,
    });
      ...
      complete: async () =>
        JSON.stringify({
          candidates: [
            {
              kind: "semantic",
              fact: "Deployment timing preferences were discussed",
              keywords: ["deploy"],
              evidence: [`runs/${RUN}#s9`],
            },
          ],
        }),
    });
    const pNo = new MemoryWritePipeline({
      correctionConfirmer: { confirm: async () => false },
      distiller,
    });
      ...
      complete: async () =>
        JSON.stringify({
          candidates: [
            {
              kind: "semantic",
              fact: "Sunstone fact that must not be silently dropped",
              keywords: [],
              evidence: [`runs/${RUN}#s7`],
            },
          ],
        }),
    });
  ...
  it("#13 gc 写 op-log；diff 三口径（新增/更新/失效/物理删除）", async () => {
    ...
    const updated = makeSemantic("Ulexite entry with history chain", {
      tValid: old,
      created: old,
    });
    ...
    await engine.put({
      ...updated,
      history: [{ fact: "old version", tInvalid: new Date().toISOString() }],
    });
