    ...
    const p = buildJudgePrompt(
      {
        ...
        events: [{ type: "action_failed", errorOutput: "DeployError: x" }],
        outcome: "failed",
      },
      "经验正文",
    );
...
function makeEpisodic(whenToUse: string, perspective: string): EpisodicExperience {
  ...
  return {
    ...
    whenToUse,
    perspective,
    modification: ["Check rotation windows first"],
    issueType: "DeployError",
    taskId: "tsk_m8",
  };
}
...
describe("shadow 模式 + 轨迹回放 db 集成", () => {
  ...
  it("shadow：假设注入包完整记录但不注入（freq 不涨、无 read.inject、无 memory.inject 事件）", async () => {
    const e = makeEpisodic(
      "When zephyr deploys fail with token mismatch",
      "Zephyr token mismatches come from stale rotation windows",
    );
    await engine.put(e);
    ...
    const real = new TriggeredRetriever({
      engine,
      countTokens: (t) => Math.ceil(t.length / 4),
    });
  ...
  it("轨迹回放：3 条样例轨迹 + mock judge → Δ 报告（helpful/neutral/harmful 各一）", async () => {
    ...
    const judge: JudgeLlm = {
      complete: async (prompt) => {
        if (prompt.includes("stale rotation windows"))
          return JSON.stringify({ verdict: "helpful", reason: "直击根因" });
        if (prompt.includes("rotate zephyr tokens during deployment"))
