...
describe("#17 failureFixPair 参数化校验", () => {
  ...
  test("轨迹含失败→成功转折时 episodic 缺 failureFixPair → 校验失败", () => {
    expect(
      hasFailureToSuccessTurn({
        outcome: "success",
        trajectory: "first attempt failed with Error X, then fixed",
      }),
    ).toBe(true);
  ...
  test("distill 端到端：转折轨迹 + 缺 fixPair 的 LLM 输出 → 重试后降级", async () => {
    const llm: DistillerLlm = {
      complete: async () => JSON.stringify({ candidates: [episodic] }),
    };
    const d = new MemoryDistiller(llm);
    const r = await d.distill({
      runId: "r1",
      goal: "g",
      trajectory: "first attempt failed with TypeError, then succeeded",
      outcome: "success",
    });
...
describe("#22 stats 成本渲染", () => {
  function stats(calls: number): MemoryStats {
    return {
      ...
      injected30d: 0,
      adopted30d: 0,
      todayDistillCalls: calls,
      todayEstimatedTokens: 12345,
    };
  }
...
describe("修复批次 C db 集成", () => {
  ...
  it("#14 冒烟不抽 degraded 条目（扫描与冒烟同口径）", async () => {
    ...
    const deg = {
      ...makeSemantic("Degraded smoke exclusion probe entry"),
      degraded: true,
    } as SemanticFact;
  ...
  it("#20 replay 的 shadow 强制不可覆盖", async () => {
    ...
    const report = await runReplay(
      [
        {
          taskId: `${RUN}_shadow`,
          description: "replay shadow enforcement probes run",
          repo: REPO,
          events: [],
        },
      ],
    ...
    const shadows = await queryOpLog({
      runId: `${RUN}_shadow`,
      op: "read.shadow",
    });
  ...
  it("#22 stats 聚合今日蒸馏成本", async () => {
    await appendOpLog("write.distill", {
      runId: `${RUN}_cost`,
      detail: { estimatedTokens: 500 },
    });
    await appendOpLog("write.enqueued", {
      runId: `${RUN}_cost`,
      detail: { eventType: "task_succeeded", estimatedTokens: 300 },
    });
