...
export class SelfEvolvingLoop {
  ...
  private async createCandidate(
    ...
    await sql`
      ...
      VALUES (${id}, ${batchId}, ${type}, ${textArrayLiteral(targetIds)}::text[], ${`Auto-${type}: ${targetIds.length} memories`}, ${`Generated by SelfEvolvingLoop. Score: ${score.overall.toFixed(2)}, suggestion: ${score.suggestion}`}, ${sql.json(evidence as any)}, ${score.overall}, ${type === "DEPRECATE" ? "medium" : "low"}, ${sql.json(evidence as any)}, 'generated', ${sql.json({ actorType: "system", actorId: "self-evolving-loop" } as any)}, ${now})
    `;
    return {
      id,
      batchId,
  ...
  private async submitToGovernance(cand: EvolutionCandidate): Promise<{ id: string } | null> {
    ...
    try {
      await sql`
        ...
        VALUES (${id}, 1, 'draft', 'project_knowledge', ${`evolved:${cand.evolutionType}:${cand.batchId}`}, 1, ${cand.proposedTitle}, ${cand.proposedSummary}, ${sql.json(cand.proposedPayload as any)}, ${sql.json({ lifecycleScope: "persistent" } as any)}, ${cand.proposedConfidence}, ${textArrayLiteral([])}::text[], ${sql.json([] as any)}, ${sql.json([] as any)}, ${textArrayLiteral(cand.targetMemoryIds)}::text[], ${textArrayLiteral([])}::text[], ${cand.riskLevel}, ${cand.riskLevel !== "low"}, ${sql.json(cand.generatedBy as any)}, ${`auto-${cand.evolutionType}`}, 'internal', ${now}, ${now})
