  ...
  async check(
    ...
    if (!this.adapter || !this.adapter.isAvailable()) {
      return {
        ...
        status: "UNKNOWN",
        expectedFact: "N/A",
        observedFact: "Code index unavailable",
        evidence: [],
        confidence: 0.0,
        checkedAt: now,
      };
    ...
    try {
      ...
      for (const filePath of relatedFiles.slice(0, 5)) {
        ...
        if (results.length === 0) {
          evidence.push({
            filePath,
            detail: "File not found in current index",
          });
        } else {
          evidence.push({
            filePath,
            detail: `Found: ${results[0]!.data.reason}`,
          });
        }
      }
      ...
      if (evidence.length === 0) {
        return {
          ...
          status: "UNKNOWN",
          expectedFact: "",
          observedFact: "No file references to check",
          evidence: [],
          confidence: 0.0,
          checkedAt: now,
        };
      ...
      if (missing.length === evidence.length) {
        return {
          ...
          subjectKey,
          status: "MISSING",
          expectedFact: "All referenced files should exist",
          observedFact: `${missing.length} files not found`,
          evidence,
          confidence: 0.8,
          checkedAt: now,
        };
      ...
      if (found.length > 0 && missing.length === 0) {
        return {
          ...
          subjectKey,
          status: "CONSISTENT",
          expectedFact: "Referenced files should exist",
          observedFact: `All ${found.length} files found`,
          evidence,
          confidence: 0.6,
          checkedAt: now,
        };
      ...
      return {
        ...
        expectedFact: "All files should exist",
        observedFact: `${found.length} found, ${missing.length} missing`,
        evidence,
        confidence: 0.5,
        checkedAt: now,
      };
