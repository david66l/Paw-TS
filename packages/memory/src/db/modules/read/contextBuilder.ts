...
import type { ContextPlacement, WorkingMemory } from "../../types.js";
import { type ContextPolicy, PolicyEngine } from "../platform/policyEngine.js";
...
export class ContextBuilder {
  ...
  build(input: ContextBuildInput): ContextBuildResult {
    ...
    if (input.workingMemory.goal) {
      ...
      items.push(
        this.makeItem("working_memory", "goal", "hot", content, ++order, {
          authority: 1.0,
          relevance: 1.0,
          freshness: 1.0,
          confidence: 1.0,
    ...
    // Plan (top 5 steps)
    const activeSteps = input.workingMemory.plan.filter(
    ...
    if (input.workingMemory.diffSummary) {
      ...
      items.push(
        this.makeItem("working_memory", "diff", "hot", content, ++order, {
          authority: 1.0,
          relevance: 0.9,
          freshness: 1.0,
          confidence: 1.0,
  ...
  private applyBudget(
    ...
    for (const item of tierItems) {
      if (used + item.estimatedTokens > budget) {
        ...
        if (placement === "hot") {
          item.placement = "warm";
          warnings.push(`Downgraded hot→warm: ${item.sourceId}`);
        } else if (placement === "warm") {
          item.placement = "cold_pointer";
          warnings.push(`Downgraded warm→cold: ${item.sourceId}`);
        } else {
          item.placement = "excluded";
          warnings.push(`Excluded: ${item.sourceId}`);
        }
      } else {
