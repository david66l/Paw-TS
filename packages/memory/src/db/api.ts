  ...
  try {
    ...
    if (path === "/memories" && method === "POST") {
      ...
      const item = {
        ...
        createdBy: {
          actorType: "user",
          actorId: (body.userId as string) ?? "api",
        },
        updatedBy: {
          actorType: "user",
          actorId: (body.userId as string) ?? "api",
    ...
    if (path === "/context/build" && method === "POST") {
      ...
      const ctx = ctxBuilder.build({
        ...
        tokenBudget:
          ...
          engine.getDefaults().context.tokenBudget.availableForContext,
      });
      return json(ctx);
    }
    ...
    if (path === "/candidates" && method === "GET") {
      const url = new URL(req.url);
      const list = await admin.listPendingCandidates(
    ...
    if (
      ...
      const decision = await admin.rejectCandidate(
        id,
        (body.reason as string) ?? "Rejected via API",
      );
    ...
    if (path.match(/^\/tasks\/([^/]+)\/attempts$/) && method === "GET") {
      ...
      const rows =
        await sql`SELECT * FROM task_execution_attempts WHERE task_id = ${taskId} ORDER BY attempt_number`;
    ...
    if (path.match(/^\/tasks\/([^/]+)\/retry$/) && method === "POST") {
      ...
      const sql = (await import("./connection.js")).getSql();
      const max =
        await sql`SELECT COALESCE(MAX(attempt_number),0)+1 AS n FROM task_execution_attempts WHERE task_id = ${taskId}`;
...
const port = Number.parseInt(process.env.MEMORY_API_PORT ?? "3300");
