...
export function shouldArchiveEntry(
  ...
  // 计算年龄截止线：距今 maxAgeDays 天之前的时间戳
  const ageCutoff = now - maxAgeDays * 24 * 60 * 60 * 1000;

  // 规则 1：低优先级 + 超龄
  // 默认优先级视为 "mid"，不属于低优先级不会被此规则触发
  if ((entry.priority ?? "mid") === "low") {
    // 优先使用 updatedAt（最近更新），没有则用 createdAt
    const ts = entry.updatedAt ?? entry.createdAt;
    if (ts !== undefined && ts <= ageCutoff) {
      return true;
    }
  }

  // 规则 2：显式有效期已过
  // valid_until > 0 表示设置了有效期（0 或 undefined 表示永不过期）
  if (
...
export function archiveExpiredEntries(
  ...
  // 在循环外部获取 now，确保同批次内所有条目使用相同时间基准
  const now = Date.now();
  const archivedNames: string[] = [];

  for (const entry of entries) {
    if (shouldArchiveEntry(entry, now, maxAgeDays)) {
      // archiveEntryByName 内部处理文件不存在的情况
