...
export interface AutoMemoryEntry {
  ...
  /** Coding-agent 记忆细分类。旧文件缺失时由 type 映射得到。 */
...
export class AutoMemoryStore {
  ...
  load(name: string): AutoMemoryEntry | null {
    ...
    try {
      ...
      const evidence = parseCsvList(fm.evidence);
      const validUntil = fm.valid_until ? Number(fm.valid_until) : undefined;
      const symbols = parseCsvList(fm.symbols);
      const tests = parseCsvList(fm.tests);
      const supersedes = parseCsvList(fm.supersedes);
  ...
  upsert(entry: AutoMemoryEntry): "created" | "updated" {
    ...
    if (existing) {
      ...
      this.save({
        ...
        kind: entry.kind ?? prior?.kind,
        confidence: entry.confidence ?? prior?.confidence,
        ...
        gitCommit: entry.gitCommit ?? prior?.gitCommit,
        branch: entry.branch ?? prior?.branch,
        symbols: entry.symbols ?? prior?.symbols,
        tests: entry.tests ?? prior?.tests,
        supersedes: entry.supersedes ?? prior?.supersedes,
