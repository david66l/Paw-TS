...
export function formatRunMetricsSummary(m: RunMetrics): string {
  ...
  const dur = m.durationMs >= 1000 ? `${(m.durationMs / 1000).toFixed(1)}s` : `${m.durationMs}ms`;
  // 智能格式化 token 数：>= 1000 用 K
  const tok = m.totalTokens >= 1000 ? `${(m.totalTokens / 1000).toFixed(1)}K` : `${m.totalTokens}`;
  // 仅当有工具调用时才显示工具成功率
  const tools = m.toolCalls > 0 ? ` · ${m.toolSuccesses}/${m.toolCalls} tools` : "";
  // 仅当有截断时才显示截断信息
