...
export async function resolveEvidencePass(input: {
  ...
  const sourceLocalRequirements = input.requirements.map((requirement, index) => {
  ...
  const certifiedAssistantDialogueCandidate = memoryQueryAnswerOriginAllowsLateBindingV1(
    input.queryAnswerOrigin,
  );
  const evidenceGroundedRoleBindingEligible =
    input.evidenceGroundedRoleBinding === true && certifiedAssistantDialogueCandidate;
  ...
  const leafRoles = new Set(input.requirements.map((requirement) => requirement.roleConstraint));
  ...
  const originalLaneUsesAuthorizedUnfiltered = evidenceGroundedRoleBindingEligible;
  const acquisition = input.sourceLock
    ...
    : buildMemoryRequirementFairAcquisitionV1({
        ...
        originalLaneMode: originalLaneUsesAuthorizedUnfiltered
          ...
          : "role_filtered",
        original: originalLaneUsesAuthorizedUnfiltered ? input.primaryUnfiltered : input.primary,
  ...
  const degradedChannels =
    ...
    (Object.freeze(
      [
        ...new Set(
          [
            originalLaneUsesAuthorizedUnfiltered ? input.primaryUnfiltered : input.primary,
            ...supplemental,
  ...
  const ordinaryLeafEligibility = sourceLocalRequirements.map((requirement, index) =>
    evaluateMemorySourceLocalLeafEligibilityV2({
      requirement,
      temporalBindingRevision: boundTemporalConstraints[index]?.bindingRevision ?? "",
  ...
  const roleBindingLeafEligibility = roleBindingRequirements.map((requirement, index) =>
    evaluateMemorySourceLocalLeafEligibilityV2({
      requirement,
      temporalBindingRevision: boundTemporalConstraints[index]?.bindingRevision ?? "",
  ...
  const effectiveLeafEligibility: readonly MemorySourceLocalLeafEligibilityV2[] =
    evidenceGroundedRoleBindingEligible ? roleBindingLeafEligibility : ordinaryLeafEligibility;
  ...
  const planScopedLocalization = sourceLocalExecutionRoute.executor === "plan_scoped_v24";
  const localEligible =
  ...
  const dialogueCandidateSourceIds =
    legacyDialogueDiscoveryEligible && !input.sourceLock
      ? rankMemoryEvidenceCandidatesV2({
          ...
          maxSources: Math.min(input.maxSources, MAX_DIALOGUE_DISCOVERY_SOURCES_V1),
          maxEvidencePerSource: input.maxEvidencePerSource,
  ...
  const baselineRequirementHits = input.requirements.map((requirement, index) =>
    mergeEvidenceHits(
      input.sourceLock ? input.sourceLock.seedHits : (supplemental[index]?.hits ?? []),
      input.sourceLock
        ? []
        : filterEvidenceSearchResultForRole(input.primaryUnfiltered, requirement.roleConstraint)
            .hits,
  ...
  let certifiedDialogueProofsByAssistant = new Map<string, MemoryDialoguePredecessorProofV1>();
  ...
  if (localEligible && sourceLocalLockedIds.length > 0) {
    if (!input.sourceLocalLocator) {
      ...
      const leafReports: MemorySourceLocalLeafExecutionReportV2[] = effectiveLeafEligibility.map(
        (eligibility, index) =>
          Object.freeze({
            eligibility,
            status: "not_attempted" as const,
      ...
      const locatorExecutionRequirements = planScopedLocalization
        ? input.requirements
        : sourceLocalRequirements;
      for (const [requirementIndex, requirement] of locatorExecutionRequirements.entries()) {
        const eligibility = effectiveLeafEligibility[requirementIndex];
        if (!planScopedLocalization && !eligibility?.eligible) continue;
        if (!eligibility) throw namedError("MemorySourceLocalEligibilityMissing");
        try {
          const locatorRequirement =
            evidenceGroundedRoleBindingEligible && roleBindingRequirements[requirementIndex]
              ? roleBindingRequirements[requirementIndex]
              : requirement;
          const materializationMode: MemoryQueryAnswerOriginMaterializationModeV1 | undefined =
            evidenceGroundedRoleBindingEligible
              ? "late_binding"
              : requirement.roleConstraint === "assistant"
                ? "assistant_leaf"
                : requirement.roleConstraint === "any" && input.intent.roleConstraint !== "user"
                  ? "shared_envelope"
                  : undefined;
          ...
          const sourceFairAnchorCount = Math.min(
            MAX_RESPONDING_ASSISTANT_CANDIDATES_V1,
            respondingAssistantSourceIds.length * RESPONDING_ASSISTANT_PROMPT_ANCHORS_PER_SOURCE_V1,
          );
          const locatorBudget = respondingAssistantMaterializationEligible
            ? Object.freeze({
                ...baseBudget,
                maxAnchors: Math.max(baseBudget.maxAnchors, sourceFairAnchorCount),
                maxChars: Math.max(
                  baseBudget.maxChars,
            ...
            ...(respondingAssistantMaterializationEligible
              ? {
                  respondingAssistantMaterialization: Object.freeze({
                    ...
                    sourcePriorityIds: Object.freeze([...respondingAssistantSourceIds]),
                    maxPromptAnchorsPerSource: RESPONDING_ASSISTANT_PROMPT_ANCHORS_PER_SOURCE_V1,
          ...
          const locatedResult = await input.sourceLocalLocator.locate(request, input.signal);
          validateMemorySourceLocalEvidenceResultV1({
          ...
          leafReports[requirementIndex] = Object.freeze({
            ...
            baselineHitCount: baselineRequirementHits[requirementIndex]?.length ?? 0,
          ...
          if (planScopedLocalization) {
            ...
            for (const [index, reportEligibility] of effectiveLeafEligibility.entries()) {
      ...
      const telemetry = Object.freeze({
        ...
        anchorCount: results.reduce((total, result) => total + result.telemetry.anchorCount, 0),
        includedTurnCount: results.reduce(
          (total, result) => total + result.telemetry.includedTurnCount,
          0,
        ),
        renderedChars: results.reduce((total, result) => total + result.telemetry.renderedChars, 0),
        cacheHit: results.every((result) => result.telemetry.cacheHit),
        durationMs: results.reduce((total, result) => total + result.telemetry.durationMs, 0),
      });
      const failedLeafCount = leafReports.filter(
        (report) => report.status === "fallback" || report.status === "invalid_result",
      ).length;
      const invalidLeaf = leafReports.find((report) => report.status === "invalid_result");
  ...
  let supportAssessments: readonly Readonly<MemoryEvidenceTriageAssessmentV1>[] = Object.freeze([]);
  let selectorExecutionSnapshot: MemorySelectorExecutionSnapshotV1 | undefined;
  let executionRequirements = input.requirements;
  let selectedRefsByRequirement: ReadonlyMap<string, ReadonlySet<string>> | undefined;
  ...
  if (input.requirements.length > 0 && input.supportSelector) {
    ...
    selectedRefsByRequirement = new Map(
      input.requirements.map((requirement) => [requirement.requirementId, new Set<string>()]),
    );
    ...
    if (candidates.length > 0) {
      const candidateEvidenceRefs = new Set(candidates.map((candidate) => candidate.evidenceRef));
          ...
          Object.freeze({
            ...
            evidenceRefs: Object.freeze(
              (requirementHits[index] ?? [])
                ...
                .filter((evidenceRef) => candidateEvidenceRefs.has(evidenceRef)),
            ),
          }),
        ),
      );
      ...
      if (verifierTargets.length > 0 && input.dialoguePredecessorVerifier) {
        try {
          ...
          const verificationResult = await input.dialoguePredecessorVerifier.verify(
            request,
            input.signal,
          );
          const verifiedProofs = validateMemoryDialoguePredecessorVerificationV1({
            verifier: input.dialoguePredecessorVerifier,
            request,
            result: verificationResult,
            evidenceRefBelongsToSource: input.index.evidenceRefBelongsToSource,
          });
          certifiedAssistantDialogueRefs = new Set([
          ...
          dialogueVerificationRevision = verificationResult.verificationRevision;
          ...
          const exactAssistantBindingByRef = new Map(
            candidates
              .filter((candidate) => proofByAssistantRef.has(candidate.evidenceRef))
              .map((candidate) => [candidate.evidenceRef, candidate] as const),
          );
          requirementHits = Object.freeze(
            requirementHits.map((hits) =>
              Object.freeze(
                hits.map((hit) => exactAssistantBindingByRef.get(hit.evidenceRef) ?? hit),
              ),
            ),
          );
        } catch (error) {
      ...
      const structurallyBoundCertifiedAssistantDialogueRefs = new Set(
        [...certifiedAssistantDialogueRefs].filter((evidenceRef) => {
          const candidate = candidates.find((item) => item.evidenceRef === evidenceRef);
          return (
            candidate?.authority === "context_only" &&
            candidate.sourceKind === "assistant_output" &&
      ...
      const selectorCertifiedAssistantDialogueRefs = selectScopedCertifiedAssistantDialogueRefsV1({
        certifiedEvidenceRefs: structurallyBoundCertifiedAssistantDialogueRefs,
      ...
      if (evidenceGroundedRoleBindingEligible) {
        ...
        executionRequirements = Object.freeze(
          input.requirements.map((requirement) => {
            ...
            const hasCertifiedAssistantAlternative = scope?.evidenceRefs.some((evidenceRef) =>
              certifiedSet.has(evidenceRef),
            );
            if (!hasCertifiedAssistantAlternative) return requirement;
            const roleCandidates = Object.freeze([
              ...(requirement.roleConstraint === "assistant" ? ([] as const) : (["user"] as const)),
              "assistant" as const,
            ]);
            return Object.freeze({
              ...requirement,
      ...
      try {
        const settlement = await settleMemoryEvidenceSupportSelectionV1({
          ...
          requirementHits,
          roleConstraint: input.intent.roleConstraint,
          certifiedSharedDialogueRefs: structurallyBoundCertifiedAssistantDialogueRefs,
          ...
          originRevision: input.queryAnswerOrigin.originRevision,
          committedAttempt: localEvidenceRefs.size > 0 ? "augmented" : "baseline",
          attemptCount: 1,
          signal: input.signal,
        });
        ...
        supportSelectorStatus = settlement.failedGroupCount > 0 ? "partial" : "completed";
        sourceLocalization = Object.freeze({
          ...sourceLocalization,
          ...(settlement.failedGroupCount > 0
            ...
            : {}),
          selectorAttempts: 1,
          selectorCommittedAttempt: localEvidenceRefs.size > 0 ? "augmented" : "baseline",
        });
      } catch (error) {
        if (input.signal.aborted || isAbort(error)) throw abortError();
        const baselineRetryEligible =
          sourceLocalExecutionRoute.reasonCode === "recommendation_operand_materialization" &&
          localEvidenceRefs.size > 0 &&
        ...
        if (baselineRetryEligible) {
          ...
          if (baselineCandidates.length > 0) {
                ...
                Object.freeze({
                  ...
                  evidenceRefs: Object.freeze(
                    (baselineRequirementHits[index] ?? [])
                      .map((hit) => hit.evidenceRef)
                      .filter((evidenceRef) => baselineCandidateEvidenceRefs.has(evidenceRef)),
                  ),
                }),
              ),
            );
            try {
              ...
              const fallbackSettlement = await settleMemoryEvidenceSupportSelectionV1({
                selector: input.supportSelector,
                query: input.query,
                intent: input.intent,
                requirements: input.requirements,
              ...
              selectedRefsByRequirement = fallbackSettlement.selectedRefsByRequirement;
              supportSelectionRevision = fallbackSettlement.selectionRevision;
              selectorExecutionSnapshot = fallbackSettlement.selectorExecutionSnapshot;
  ...
  if (
    ...
    const failedCandidateRefs = new Set(failedCandidates.map((candidate) => candidate.evidenceRef));
    ...
    selectorExecutionSnapshot = compileMemorySelectorExecutionSnapshotV1({
      ...
      groups: compileMemoryEvidenceSelectorGroupsV1({
        ...
        Object.freeze({
          ...
          failureCodes: Object.freeze(["MemoryEvidenceSupportSelectionNotCommitted"]),
        }),
      ),
    });
  }
  ...
  if (selectedRefsByRequirement !== undefined) {
    const floor = applyMemoryDeterministicSupportFloorV1({
      ...
      excludedEvidenceRefs: supportAssessments.map(
        (assessment) =>
          new Set([...assessment.contradictingEvidenceRefs, ...assessment.unknownEvidenceRefs]),
      ),
    });
  ...
  const notebook = buildMemoryEvidenceNotebookV1({
    requirements: executionRequirements.map((requirement, index) => ({
      ...
      label:
        meaTimeWindowSuffix === ""
          ? requirement.label
          : requirement.label.slice(0, 192 - meaTimeWindowSuffix.length) + meaTimeWindowSuffix,
      searchText: requirement.searchText,
      ...
      roleConstraint: requirement.roleConstraint,
      certifiedDialogueEvidenceRefs: Object.freeze([...certifiedAssistantDialogueRefs]),
  ...
  const packetFallbackHits = mergeEvidenceHits(
    requirementHits
      .flat()
      .filter(
        (hit) => nonSupportingRefs.has(hit.evidenceRef) && !localEvidenceRefs.has(hit.evidenceRef),
      ),
    input.primary.hits,
  ).filter((hit) => !input.excludedEvidenceRefs?.has(hit.evidenceRef));
  const packetSources =
  ...
  const dialogueCertificateRegistry = compileMemoryDialogueCertificateRegistryV1({
    lockedSourceIds: sourceLocalLockedIds,
  ...
  return Object.freeze({
    ...
    supportAssessments,
    ...(selectorExecutionSnapshot === undefined ? {} : { selectorExecutionSnapshot }),
    sourceLocalization,
    lockedSourceIds: sourceLocalLockedIds,
    notebook,
    requirementEvidence,
...
> {
  const candidateEvidenceRefsByRequirement = new Map(
    input.candidateScopes.map((scope) => [scope.requirementId, new Set(scope.evidenceRefs)]),
  );
  ...
  if (input.selector.selectGrouped && selectorGroups.length > 1) {
    const groupedSelection = await input.selector.selectGrouped(
      {
        ...
        ...(input.selectorCertifiedAssistantDialogueRefs.length > 0
          ? {
              certifiedAssistantDialogueEvidenceRefs: input.selectorCertifiedAssistantDialogueRefs,
            }
          : {}),
      },
    ...
    const groupResultById = new Map(groupedSelection.groups.map((group) => [group.groupId, group]));
    if (
    ...
    const assessmentsByRequirement = new Map<string, Readonly<MemoryEvidenceTriageAssessmentV1>>();
    ...
    for (const group of selectorGroups) {
      ...
      const groupCandidateRefs = new Set(groupScopes.flatMap((scope) => scope.evidenceRefs));
      ...
      const groupCandidateEvidenceRefsByRequirement = new Map(
        groupScopes.map((scope) => [scope.requirementId, new Set(scope.evidenceRefs)]),
      );
      const assessments = enforceSelectedEvidenceAuthority({
        assessments: result.assessments,
        requirements: group.requirements,
        candidateEvidenceRefs: new Set(groupCandidates.map((candidate) => candidate.evidenceRef)),
        candidateEvidenceRefsByRequirement: groupCandidateEvidenceRefsByRequirement,
        ...
        certifiedDialoguePredecessorsByAssistant: new Map(
          [...input.certifiedDialoguePredecessorsByAssistant].filter(([assistantEvidenceRef]) =>
    ...
    if (committedGroupCount === 0) {
      throw namedError(
        [...new Set(failureCodes)].sort()[0] ?? "MemoryEvidenceSupportGroupSelectionEmpty",
      );
    }
    const assessments = Object.freeze(
      input.requirements.flatMap((requirement) => {
        const assessment = assessmentsByRequirement.get(requirement.requirementId);
  ...
  const assessments = enforceSelectedEvidenceAuthority({
    ...
    requirements: input.requirements,
    candidateEvidenceRefs: new Set(input.candidates.map((candidate) => candidate.evidenceRef)),
    ...
    certifiedDialoguePredecessorsByAssistant: input.certifiedDialoguePredecessorsByAssistant,
    certifiedAssistantDialogueCandidate: input.certifiedAssistantDialogueCandidate,
  });
  return Object.freeze({
  ...
  return Object.freeze(
    [...input.certifiedEvidenceRefs].filter(
      (evidenceRef) =>
        ...
        input.candidateScopes.some((scope) => scope.evidenceRefs.includes(evidenceRef)),
    ),
  );
}
        ...
        hit.includedTurns
          .filter(
            (turn) =>
              ...
              hasMemorySourceLocalDialogueCertificateV1(hit.includedTurns, turn.turnOrder),
          )
