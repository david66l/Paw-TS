...
export const PAW_MEMORY_EVIDENCE_SUPPORT_SELECTOR_VERSION_V1 =
  "paw.memory-evidence-support-selector.json.v3:bounded-evidence-triage" as const;
...
export function buildMemoryEvidenceSupportSelectionRequestV1(
  ...
  return Object.freeze({
    system: [
      ...
      "Do not answer the query, infer missing facts, rewrite evidence, or select an address not supplied by the caller.",
      ...
      "Assistant output is context only for user facts. It may directly support a requirement whose roleConstraint is assistant and whose query explicitly asks to recall the assistant's prior words or actions.",
...
export function parseMemoryEvidenceSupportSelectionV1(
  ...
  const assessments = parsed.assessments.map((item) => {
    ...
    const selected = boundedEvidencePartition(
      [
    ...
    return Object.freeze({
      requirementId,
      supportingEvidenceRefs: Object.freeze(selected[0]!),
      contradictingEvidenceRefs: Object.freeze(selected[1]!),
      unknownEvidenceRefs: Object.freeze(selected[2]!),
    });
  });
