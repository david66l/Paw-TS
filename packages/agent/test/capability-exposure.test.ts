...
describe("capability exposure shadow v1", () => {
  ...
  test("builds a stable categorized inventory", () => {
    ...
    expect(inventory.find((entry) => entry.name === "workspace.run_shell")?.category).toBe(
      "execution",
    );
    expect(inventory.find((entry) => entry.name === "workspace.web_search")?.category).toBe(
      "external",
    );
    expect(inventory.find((entry) => entry.name === "workspace.list_dir")?.core).toBe(true);
  });

  test("supports exact selection and keyword discovery", () => {
    const inventory = inventoryCapabilitiesV1(definitions, toolNameMap);
    expect(
      searchCapabilitiesV1(inventory, "select:workspace.notebook_edit").map((entry) => entry.name),
  ...
  test("estimates savings without changing provider-visible exposure", () => {
    ...
    const hit = shadow.observe(0, "fix the parser", ["workspace.read_file", "workspace.edit_file"]);
    expect(hit.outcome).toBe("hit");
    expect(hit.exposedToolCount).toBe(snapshot.fullToolCount);

    const fallback = shadow.observe(1, "fix the parser", ["workspace.web_search"]);
  ...
  test("adds git status only when the host phase makes it relevant", () => {
    ...
    expect(shadow.suggestedTools("fix the parser", initial)).toContain("workspace.git_status");

    const editing = capabilityPhaseToolsV1({
      mutationRevision: 1,
      diffInspectedRevision: 0,
    });
    expect(shadow.suggestedTools("fix the parser", editing)).not.toContain("workspace.git_status");

    const finishing = capabilityPhaseToolsV1({
      mutationRevision: 1,
      diffInspectedRevision: 1,
    });
    expect(shadow.observe(4, "fix the parser", ["workspace.git_status"], finishing).outcome).toBe(
      "hit",
    );
  });
});
