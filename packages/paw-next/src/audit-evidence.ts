...
function localPath(root: string, name: unknown): string | undefined {
  ...
  if (
    ...
    relative.split(path.sep).some((p) => [".git", ".paw"].includes(p))
  )
    return;
...
export function projectAuditorReadsV1(root: string, facts: readonly InputFactV1[]) {
  ...
  for (const fact of facts) {
    if (
      ...
      if (name)
        calls.set(fact.callId, {
          path: name,
          callId: fact.callId,
          requestedRange: Object.fromEntries(
            Object.entries(args).filter(
              ([key, value]) => key !== "path" && typeof value === "number",
            ),
          ),
        });
    } else if (
...
export function auditReportCorrectionV1(
  root: string,
  ...
  let problem: string;
  if (!isAuditReportV1(report, browserAudit))
    problem = "The report does not match the required JSON contract.";
  else {
    const missing = report.evidencePaths.filter((name) => {
      const normalized = localPath(root, name);
      return !normalized || !owned.has(normalized);
    });
    ...
    problem = missing.length
      ? `These citations are not successful reads in your child journal: ${JSON.stringify(missing)}.`
      : "No auditor-owned file evidence was cited.";
  }
  return (
    `[${AUDIT_CORRECTION_CALLER}] Your report was rejected by host validation. ${problem}\n` +
