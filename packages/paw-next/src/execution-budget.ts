...
export function executionRequestFloorMsV1(snapshot: SessionInputSnapshot<InputFactV1>): number {
  ...
  for (const { fact } of snapshot.entries) {
    if (fact.type === "execution.budget_observed" && fact.inputId === inputId) {
      ...
      completed = false;
    } else if (start !== undefined && fact.type === "model.settled" && fact.status === "completed") {
      completed = true;
    }
  }
    ...
    async reportSafeBoundary(boundary) {
      ...
      if (
        ...
        (previous.deadlineAtMs !== supplied.deadlineAtMs ||
          previous.reserveMs !== supplied.reserveMs || previous.admissionPolicy !== supplied.admissionPolicy)
      )
      ...
      if (deadline.admissionPolicy === "recent_round_floor_v1") {
        ...
        if (deadline.deadlineAtMs - observedAtMs < floor)
          throw new Error(`ExecutionBudgetInsufficientForRequest: remaining=${deadline.deadlineAtMs - observedAtMs}ms, recentRoundFloor=${floor}ms`);
      }
    },
  };
}
...
export function projectExecutionBudgetV1(
  ...
  return {
    ...
    placement: "tail",
    content: `[Paw execution budget v1] Host-observed remaining time: ${Math.ceil(remainingMs / 1000)} seconds at this request boundary. This decreases while the model and tools run. ${closeout
      ...
      : "Make incremental progress on the smallest useful end-to-end path. Allow time for its direct verification, remaining required deliverables and completion review; model-call allowance is not remaining wall time."} This observation changes no requirements or permissions.`,
  };
}
