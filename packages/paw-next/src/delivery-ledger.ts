...
import {
  type AgentAcceptanceUpdateAction,
  exitStatusProvesVerification,
  parseCommandChain,
...
function validate(input: unknown): asserts input is Update {
  ...
  if (
    !value ||
    Object.keys(value).sort().join() !== "add,reason,updates" ||
    !Array.isArray(value.add) ||
    !Array.isArray(value.updates) ||
    value.add.length + value.updates.length === 0 ||
  ...
  for (const raw of value.add) {
    ...
    if (
      ...
      (item.ref !== undefined && !text(item.ref, 300))
    )
      throw new Error("Invalid acceptance condition");
  }
  ...
  for (const raw of value.updates) {
    ...
    if (
      ...
      throw new Error("Invalid acceptance update");
    ids.add(item.id);
  }
}
...
export function createDeliveryLedgerPluginV1(): RuntimeToolPluginV1 {
  ...
  return {
    ...
    pluginId: DELIVERY_LEDGER_PLUGIN_V1,
    pluginVersion: SCHEMA,
    entries: [
      {
        internalName: ACCEPTANCE_UPDATE,
        providerName: TOOL,
        definition: {
          ...original,
          function: { ...original.function, description },
        },
        deferred: false,
        ...
        classify(_args, workspaceRoot) {
          ...
          return {
            ...
            resources: [
              {
                key: path.join(root, ".paw", "delivery-ledger"),
                access: "write",
              },
            ],
          };
...
export function inspectDeliveryLedgerV1(
  snapshot: Snapshot,
...
export function createDeliveryLedgerServiceV1(
  read: () => Promise<{
    snapshot: Snapshot;
      ...
      try {
        ...
        for (const update of input.updates) {
          ...
          if (update.status === "satisfied") {
            ...
            if (
              ...
              throw new Error(
                "satisfied needs the exact callId of current passing verification or successful readback; writes, stale checks and masked exits are insufficient",
              );
          }
        ...
        for (const item of input.add) {
          ...
          if (criteria.length === 32)
            throw new Error("At most 32 acceptance conditions per work item");
          criteria.push({
            id: `acceptance-${criteria.length + 1}`,
            text: item.text.trim(),
            source: item.source,
...
export function projectDeliveryLedgerV1(
  snapshot: Snapshot,
  evidence?: VerifiedCanonicalPayloadEvidenceV1,
  ...
  while ((header + JSON.stringify(payload)).length > 6000) {
    if (payload.references.length) {
      payload.references.shift();
      payload.omittedReferences++;
    } else if (payload.items.length) {
      payload.items.pop();
      payload.omittedItems++;
    } else break;
  }
  return {
    sourceThroughSeq: state.sourceThroughSeq,
    placement: "tail",
    content: header + JSON.stringify(payload),
  };
}
