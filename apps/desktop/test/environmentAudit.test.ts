...
test.each(["read_then_pass", "repeat_bad_report", "read_then_defect"] as const)("owned evidence correction stays in one child and is bounded: %s", async (outcome) => {
  const f = fixture("pass");
  let auditCalls = 0;
  let correctionsSeen = 0;
  const model: LanguageModel = { ...f.options.model, async complete(messages, options) {
    const text = JSON.stringify(messages);
    if (!text.includes("Paw environment auditor")) return f.options.model.complete(messages, options);
    auditCalls++;
    ...
    if (auditCalls === 2 && outcome !== "repeat_bad_report") return tool("workspace_read_file", { path: "note.txt" });
    if (auditCalls === 3) {
      expect(text).toContain("requestedRange");
      expect(text).toContain("test-call-");
    }
    return final(JSON.stringify({ completion: outcome === "read_then_defect" && auditCalls >= 3 ? "incomplete" : "complete",
      summary: "检查完成", evidencePaths: ["note.txt"], unmetCriteria: outcome === "read_then_defect" && auditCalls >= 3 ? ["内容错误，应为 checked"] : [] }));
  }};
  const options = { ...f.options, model, environmentAuditSinglePass: true, environmentAuditEvidenceRepair: true };
  ...
  await runDesktopNext("Continue", { ...options, environmentAuditEvidenceRepair: false, intent: "recover" });
  expect(auditCalls).toBe(before);
});

test.each(["pass", "turn_limit", "timeout"] as const)("audit correction retains prior reads and shares the original budget: %s", async outcome => {
  const f = fixture("pass");
  ...
  const timer = spyOn(globalThis, "setTimeout").mockImplementation(((handler, delay, ...args) => {
    ...
    return nativeTimeout(handler, delay === 240_000 && outcome === "timeout" ? 8000 : delay, ...args);
  ...
  try {
    const model: LanguageModel = { ...f.options.model, async complete(messages, options) {
      ...
      if (calls === 3 && outcome === "timeout") return new Promise((_resolve, reject) => {
        const signal = options!.signal!;
        if (signal.aborted) reject(signal.reason);
        else signal.addEventListener("abort", () => reject(signal.reason), { once: true });
      });
      if (calls === 3 || (calls > 3 && outcome === "turn_limit")) return tool("workspace_read_file", { path: "README.md" });
      if (calls === 4) {
        expect(text).toContain("Your report was rejected by host validation");
        const ledger = messages.filter(m => typeof m.content === "string" && m.content.includes("[Auditor-owned file reads]")).at(-1)!.content;
        expect(ledger).toContain("note.txt");
        expect(ledger).toContain("README.md");
      }
      return final(JSON.stringify({ completion: "complete", summary: "已核对", evidencePaths: ["note.txt", "README.md"], unmetCriteria: [] }));
    }};
    const result = await runDesktopNext("Create note.txt containing hello", { ...f.options, model, environmentAuditSinglePass: true, environmentAuditEvidenceRepair: true });
    expect(JSON.parse(result.text).acceptance, result.text).toBe(outcome === "pass" ? "verified" : "unverified");
    expect(f.counts().rootCalls).toBe(2);
    expect(timers).toBe(1);
    expect(calls).toBe(outcome === "pass" ? 4 : outcome === "turn_limit" ? 12 : 3);
    if (outcome === "pass") expect(readDesktopMonitor(f.root, "audit")?.audit?.inspected.map(i => i.path).sort()).toEqual(["README.md", "note.txt"]);
  } finally { timer.mockRestore(); }
});

test.each(["pass", "timeout", "changed"] as const)("audit timeout retries inside review without restarting root: %s", async (outcome) => {
  const f = fixture("pass");
  let auditCalls = 0;
  let rootCalls = 0;
  const nativeTimeout = globalThis.setTimeout;
  ...
  try {
      ...
      async complete(messages, options) {
        ...
        if (auditCalls === 2 || (outcome === "timeout" && auditCalls === 4)) {
          ...
          return new Promise((_resolve, reject) => {
            const stop = () => {
              if (outcome === "changed") fs.writeFileSync(path.join(f.root, "note.txt"), "external change");
        ...
        return final(JSON.stringify({ completion: "complete", summary: "Independently checked",
          evidencePaths: ["note.txt"], unmetCriteria: [] }));
      },
    };
    const result = await runDesktopNext("Create note.txt containing hello", { ...f.options, model });
    expect(JSON.parse(result.text).acceptance).toBe(outcome === "pass" ? "verified" : "unverified");
    ...
    expect(fs.readFileSync(path.join(f.root, "note.txt"), "utf8")).toBe(outcome === "changed" ? "external change" : "hello");
    if (outcome !== "changed") {
      expect(readDesktopMonitor(f.root, "audit")?.audit?.reviewId).toEndWith("-retry-1");
      const before = { rootCalls, auditCalls };
      const recovered = await runDesktopNext("Create note.txt containing hello", { ...f.options, model, intent: "recover" });
      expect(JSON.parse(recovered.text).acceptance).toBe(outcome === "pass" ? "verified" : "unverified");
  ...
  try {
    const model: LanguageModel = { ...f.options.model, async complete(messages, options) {
      ...
      if (!prompt.includes("Paw environment auditor")) {
        rootCalls++;
        return rootCalls === 1 ? tool("workspace_write_file", { path: "note.txt", content: "hello" }) : final("Everything is complete and verified.");
      }
      auditCalls++;
    ...
    expect({ rootCalls, auditCalls, auditTimers }).toEqual({ rootCalls: 2, auditCalls: 2, auditTimers: 1 });
    ...
    const recovered = await runDesktopNext("Continue", { ...options, environmentAuditSinglePass: false, intent: "recover" });
...
test("single-pass audit can verify and replay with its frozen identity", async () => {
  const f = fixture("pass");
  const result = await runDesktopNext("Create note.txt containing hello", { ...f.options, environmentAuditSinglePass: true });
  expect(result.ok, result.text).toBe(true);
  expect(f.messagesSeen.some(text => text.includes("at most 240 seconds total wall time"))).toBe(true);
  const before = f.counts();
  const recovered = await runDesktopNext("Continue", { ...f.options, intent: "recover" });
...
test("single-pass ungrounded report stops unverified without reopening the executor", async () => {
  ...
  const before = f.counts();
  const recovered = await runDesktopNext("Continue", { ...options, intent: "recover" });
  expect(JSON.parse(recovered.text).acceptance).toBe("unverified");
  expect(f.counts()).toEqual(before);
});
...
test("pre-retry audited desktop sessions recover with their original identity and zero model calls", async () => {
  const f = fixture("pass");
  const identity = { workspaceRoot: f.root, sessionId: "desktop-session-old-audit",
    runId: "desktop-next-old-audit", inputId: "desktop-input-old-audit", goal: "Create note.txt containing hello" };
  const profile = { ...desktopProfile(f.root, f.options.model, {}, undefined, false),
    auditedMemory: true as const, browserAudit: true as const };
  const requestApproval = async () => ({ decision: "allow_once" as const });
  const args = { identity, profile, model: f.options.model, apiKey: fingerprint({}), requestApproval };
  const first = buildPawNextTaskProfileV3(args);
  const resolution = buildPawNextTaskProfileV3({ ...args, profile: { ...profile, configHash: first.configHash } });
  await runFreshPawNextTaskV3({ resolution, requestApproval });
  const record = { version: 1, ...identity, liveSteering: true, environmentAudit: true,
    auditedMemory: true, browserAudit: true, configHash: resolution.configHash, status: "completed", segments: 1 };
  const dir = path.join(f.root, ".paw", "desktop-next"); fs.mkdirSync(dir, { recursive: true });
  ...
  const before = f.counts();
  const result = await runDesktopNext(identity.goal, { ...f.options, intent: "recover" });
  expect(JSON.parse(result.text)).toMatchObject({ runId: identity.runId, acceptance: "verified" });
  expect(f.counts()).toEqual(before);
});
...
test.each([false, true])("a process crash after a committed audit claim recovers without repeating the write (retry=%s)", async (retryClaim) => {
  const f = fixture("pass");
  ...
  if (retryClaim) expect(readDesktopMonitor(f.root, "audit")?.audit?.reviewId).toEndWith("-retry-1");
  expect(fs.readFileSync(path.join(f.root, "note.txt"), "utf8")).toBe("hello");
});
