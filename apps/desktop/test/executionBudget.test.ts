...
const roots: string[] = [];
function workspace() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "paw-budget-test-"));
  roots.push(root);
  return root;
}
function model(complete: LanguageModel["complete"]): LanguageModel {
  return {
    label: "openai:budget-test",
...
test("production desktop puts remaining-time evidence into the first request and keeps simple answers tool-free", async () => {
  ...
  let calls = 0;
  const result = await runDesktopNext("What is a closure?", {
    ...common,
    workspaceRoot: root,
    executionDeadline: { deadlineAtMs: Date.now() + 60_000, reserveMs: 60_000 },
  ...
  expect(record.incrementalVerification).toBe(true);
  expect(record.executionDeadline.reserveMs).toBe(60_000);
});

test("unbounded desktop has no manufactured deadline or extra model turn", async () => {
  const root = workspace();
  let calls = 0;
  const result = await runDesktopNext("Explain closures", {
    ...common,
    workspaceRoot: root,
    model: model(async (messages) => {
      calls++;
...
test("deadline cancels a live uncooperative request without replay or late tool execution", async () => {
  ...
  const result = await runDesktopNext("Implement the requested change", {
    ...
    model: model(async (_messages, options) => {
      calls++;
  ...
  ).rejects.toThrow("ExecutionDeadlineExceeded");
  await expect(
    runDesktopNext("Continue", {
      ...common,
      workspaceRoot: root,
      ...
      executionDeadline: {
        ...deadline,
        deadlineAtMs: deadline.deadlineAtMs + 60_000,
      },
      model: model(async () => {
        throw new Error("must not call");
      }),
    }),
  ).rejects.toThrow("时间预算已变化");
}, 15000);
...
test("completed bounded work recovers its identity and a new user task gets no inherited deadline", async () => {
  ...
  const recovered = await runDesktopNext("Explain closures", {
    ...options,
    intent: "recover",
  });
...
test("desktop delivery ledger links actual file readback, survives recovery and resets for new work", async () => {
  ...
  const provider = model(async (messages, options) => {
    try {
      if (messages[0]?.content.includes("completion reviewer"))
        return {
          text: '{"decision":"allow","reasonCode":"evidence_sufficient","summary":"Readback confirms the fixture"}',
          finishReason: "stop",
        };
      calls++;
      const prompt = messages.map((message) => message.content).join("\n");
      ...
      if (calls === 1)
        return invoke("workspace_acceptance_update", {
          add: [
            {
              text: "README contains Hello",
              source: "user",
              ref: "current request",
            },
          ],
          updates: [],
          reason: "Track the requested artifact",
        });
      if (calls === 2) {
          ...
          updates: [
            {
              id: "acceptance-1",
              status: "satisfied",
              evidence: state.references[0].callId,
            },
          ],
  ...
  const result = await runDesktopNext(
    "Write README.md containing Hello and verify its content",
    options,
  );
...
afterEach(() => {
  for (const root of roots.splice(0)) {
    ...
    if (
      ...
      !path.basename(resolved).startsWith("paw-budget-test-")
    )
