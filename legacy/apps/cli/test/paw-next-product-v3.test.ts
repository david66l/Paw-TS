...
} from "../../../../packages/paw-next/src/product-manifest-v2.js";
import {
...
} from "../../../../packages/paw-next/src/product-manifest-v3.js";
import {
...
const V3_HASH =
  "5afc7118d44146b3b7fe44a9a9e972739906e741e62880a8572f2cb3965eb681";
const roots: string[] = [];
...
describe("Paw Next V3 product manifest identity", () => {
  test("final tool batch semantics are frozen into manifest and profile identity", () => {
    const ordinary = createPawNextProductManifestV3(manifestInputV3());
    const input = manifestInputV3();
    const opted = createPawNextProductManifestV3({
      ...input,
      runConfig: { ...input.runConfig, settleFinalToolBatch: true },
    });
    expect(hashPawNextProductManifestV3(opted)).not.toBe(
      hashPawNextProductManifestV3(ordinary),
    );
    expect(opted.runConfig).toHaveProperty("settleFinalToolBatch", true);
    expect(ordinary.runConfig).not.toHaveProperty("settleFinalToolBatch");
  });
...
describe("Paw Next V3 profile and aggregate catalog", () => {
  ...
  test("builds V3 common identity from the existing product authority", () => {
    ...
    expect(v3Identity.registry.plugins).toEqual([
      ...
      {
        pluginId: "paw.context-compact",
        pluginVersion: "paw.context-compact.v1",
      },
      {
      ...
      },
      {
        ...
        pluginVersion:
          "paw.output-recall.v3:journal-authority:t12000:h3000:l2000:dt3000:dh1000:dl500:c8000:u32000:r256000",
      },
      {
...
function expectedManifestV3() {
  ...
  return {
    ...
    compositionVersion: "paw.product-composition.v3.27:journal-working-state",
    ...
    contextCompaction: {
      ...
      distillerPolicyVersion:
        "paw.checkpoint-distiller.v2:p256000:o4096:t45000:vrequired:hard-deadline",
      semanticVerifierPolicyVersion:
        "paw.checkpoint-semantic-verifier.v2:p192000:o512:t30000:hard-deadline",
    },
    completionReview: {
      ...
      reviewerPolicyVersion:
        "paw.completion-reviewer.v4:observed-output:p96000:o4096:r1:t30000:hard-deadline",
      continuationPolicyVersion: "paw.completion-review-continuation.v2",
      maxBlocksPerRun: 2,
      deliveryReview: "tool-free-semantic:512:no-truncation-retry",
    },
    progressAdvisor: {
      policyVersion:
        "paw.progress-advisor.v8:r3-5-8:n4-8-16:v4-8-16:repair2:closeout2:g16-18:e8:independent-closeout:journal-anchor",
      mode: "journal_anchored_advice_with_budgeted_active_fallback",
      workingStatePolicyVersion: "paw.working-state.v1:journal:bounded:conservative-freshness",
    },
    collaboration: {
      policyVersion:
        "paw.collaboration.v1.8:boundaries:mission-budget:soft-renewal16:n8:c3:m8:d24:s96:t240:g4000:r6000",
    ...
    modelOutputRecovery: {
      policyVersion: "paw.model-output-recovery.v2:native:f8192:c3",
      outputLimit: "model-capability",
      fallbackMaxOutputTokens: 8192,
      maxContinuations: 3,
      requestSupervision: { policyVersion: "paw.model-request-supervision.v1:idle90000:reasoning600000:wall900000:no-retry", idleMs: 90000, reasoningOnlyMs: 600000, wallMs: 900000 },
    },
    memoryMaintenance: "paw.memory-maintenance.v1:shared-30s-deadline:staged-recovery",
    payloadRuntime: payloadRuntime(),
    ...common,
  };
}
...
test("audit retry has a distinct opt-in identity and requires auditing", () => {
  const base = { ...manifestInputV3(), environmentAudit: true as const };
  const original = createPawNextProductManifestV3(base);
  const retry = createPawNextProductManifestV3({ ...base, environmentAuditRetry: true });
  expect(original).not.toHaveProperty("environmentAuditRetry");
  expect(retry.environmentAuditRetry).toBe("paw.environment-audit-retry.v1");
  expect(hashPawNextProductManifestV3(retry)).not.toBe(hashPawNextProductManifestV3(original));
  expect(() => createPawNextProductManifestV3({ ...manifestInputV3(), environmentAuditRetry: true })).toThrow("requires");
});
...
test("single-pass audit is identity-bound while historical manifest hashes stay unchanged", () => {
  const base = { ...manifestInputV3(), environmentAudit: true as const, environmentAuditRetry: true as const };
  const old = createPawNextProductManifestV3(base);
  const current = createPawNextProductManifestV3({ ...base, environmentAuditSinglePass: true });
  expect(old.environmentAuditSinglePass).toBeUndefined();
  expect(current.environmentAuditSinglePass).toBe("paw.environment-audit-single-pass.v1");
  const evidenceRepair = createPawNextProductManifestV3({ ...base, environmentAuditSinglePass: true, environmentAuditEvidenceRepair: true });
  expect(current.environmentAuditEvidenceRepair).toBeUndefined();
  expect(evidenceRepair.environmentAuditEvidenceRepair).toBe("paw.environment-audit-evidence-repair.v1");
  expect(hashPawNextProductManifestV3(evidenceRepair)).not.toBe(hashPawNextProductManifestV3(current));
  expect(() => createPawNextProductManifestV3({ ...base, environmentAuditEvidenceRepair: true })).toThrow("requires");
  expect(hashPawNextProductManifestV3(current)).not.toBe(hashPawNextProductManifestV3(old));
  expect(() => createPawNextProductManifestV3({ ...manifestInputV3(), environmentAuditSinglePass: true })).toThrow("requires");
  expect(hashPawNextProductManifestV3(createPawNextProductManifestV3(manifestInputV3()))).toBe(V3_HASH);
});
