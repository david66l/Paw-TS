...
import { loadPawNextCollaborationRosterV1 } from "../../../../packages/paw-next/src/collaboration-roster-adapter.js";
import {
...
import {
  hashCanonicalJsonV1,
  toFrozenJsonValueV1,
} from "../../../../packages/paw-next/src/product-manifest.js";
import { createPawNextProductProfileCatalogV3 } from "../../../../packages/paw-next/src/product-profile-catalog-v3.js";
import {
...
} from "../../../../packages/paw-next/src/product-profile-v2.js";
import {
...
import { scanAndResumePawNextRunsWithCatalogV1 } from "../../../../packages/paw-next/src/startup-scan.js";
...
describe("Paw Next programmatic product-catalog startup scanner", () => {
  ...
  test("keeps the shared scanner and main free of implicit work admission seams", () => {
    const sources = [
      "../../../../packages/paw-next/src/startup-scan.ts",
      "../src/main.ts",
    ].map(
      (relative) =>
...
describe("Paw Next V3 Fresh and Existing composition skeleton", () => {
  test("audits an inspect stage through a nested read-only child", async () => {
    const root = workspace();
    fs.writeFileSync(path.join(root, "evidence.txt"), "verified evidence\n");
    const model = new ScriptedModel([
      toolResponse("stage-read", "workspace_read_file", {
        path: "evidence.txt",
      }),
      finalResponse("Read evidence.txt: verified evidence."),
      toolResponse("audit-read", "workspace_read_file", {
        path: "evidence.txt",
      }),
      finalResponse(
        JSON.stringify({
          completion: "complete",
          summary: "Confirmed evidence.txt contents",
    ...
    const resolution = resolutionV3WithModel(
      taskIdentity(root, "nested-inspect-audit"),
      model,
    );
    const agent = loadPawNextCollaborationRosterV1(root).agents.find(
      (item) => item.id === "bige",
    );
    if (!agent) throw new Error("missing bige");
    const result = await runPawNextChildV3({
      parentOptions: resolution.taskOptions,
      parentTaskOptions: resolution.taskOptions,
      callId: "inspect-stage",
      goal: "Read evidence.txt and report its actual contents without changing files.",
      agent,
      auditedStage: true,
      maxModelTurns: 3,
    });
    ...
    expect(model.requests[2]?.[0]?.content).toContain(
      "You are Environment auditor",
    );
    expect(result.environmentAudit?.status).toBe("verified");
    expect(fs.readFileSync(path.join(root, "evidence.txt"), "utf8")).toBe(
      "verified evidence\n",
    );
  });
  ...
  test("durably dispatches one roster role through the collaboration plugin", async () => {
    ...
    // Cache-stable redesign: after_boundary anchors runtime activity before
    // the timeline unit that contains its source boundary, so the native
    // tool turn (with the full bounded child result) stays append-last.
    expect(activityEvidenceIndex).toBe(2);
    ...
    // The tail [Paw Current Working State] annotation re-renders on every
    // request by design: volatile host evidence stays at the tail so the
    // provider prefix cache is untouched. Everything before it must remain
    // strictly append-only between consecutive requests.
    expect(
      model.requests[3]?.slice(0, (model.requests[2]?.length ?? 0) - 1),
    ).toEqual(model.requests[2]?.slice(0, -1));
    expect(model.requests[3]?.at(-1)?.content).toContain(
...
`,
  ...
  test("V3 anchors verification advice after repeated writes in the actual model request", async () => {
    const root = workspace();
    const identity = { ...taskIdentity(root, "v3-verification-cadence"), goal: "Create and verify four small tests." };
    const model = new ScriptedModel([
      ...Array.from({ length: 4 }, (_, index) => toolResponse(`write-${index}`, "workspace_write_file", {
        path: `check-${index}.test.js`,
        content: `const test = require('node:test'); test('check ${index}', () => {});\n`,
      })),
      toolResponse("verify", "workspace_run_shell", { command: "node --test check-0.test.js check-1.test.js check-2.test.js check-3.test.js" }),
      finalResponse("All four tests passed."),
      finalResponse(JSON.stringify({ decision: "allow", reasonCode: "evidence_sufficient", summary: "The requested checks passed." })),
    ]);
    const result = await runFreshPawNextTaskV3({ resolution: resolutionV3WithModel(identity, model, {}, true, true) });
    expect(result.assistantText).toBe("All four tests passed.");
    expect(model.requests[3]?.some((message) => message.content.includes("adviceId=verification_due:"))).toBeFalse();
    const reminder = model.requests[4]?.find((message) => message.content.includes("adviceId=verification_due:"));
    expect(reminder?.role).toBe("user");
    expect(reminder?.content).toContain("4 mutation turns");
    expect(model.requests[5]?.filter((message) => message.content.includes("adviceId=verification_due:"))).toEqual([reminder!]);
    expect(result.inputFacts.some((fact) => fact.type === "tool.settled" && fact.callId.endsWith("verify") && fact.observation?.isError === false)).toBeTrue();
  }, 30_000);
  ...
  test("requires a decisive hard-stall replan while preserving the main Agent's choice", async () => {
    ...
    const model = new ScriptedModel([
      ...
      // The replan shell (effect unknown) counts as an unverified mutation
      // under the stricter effect-aware review accounting, so the closeout
      // reviewer must settle this run with an explicit allow verdict.
      finalResponse(
        JSON.stringify({
          decision: "allow",
          reasonCode: "evidence_sufficient",
          summary: "The bounded replan checkpoint is a deliberate stop.",
        }),
      ),
    ]);
...
describe("thinking-recovery telemetry reaches the V3 run entry", () => {
  test("rescue events surface through runFreshPawNextTaskV3", async () => {
    const root = workspace();
    const identity = {
      ...taskIdentity(root, "v3-rescue-telemetry"),
      goal: "Rescue telemetry probe.",
    };
    let streamCalls = 0;
    const seed = profileV3();
    const streamProfile = {
      ...seed,
      model: { ...seed.model, transport: "stream" as const },
    };
    const first = buildPawNextTaskProfileV3({
      identity,
    ...
    const identity0 = first.taskOptions.model;
    const stallModel: LanguageModel = {
      label: identity0.label,
      capabilities: identity0.capabilities,
      runtimeProfile: identity0.runtimeProfile,
      async complete() {
      ...
      async *completeStream(messages, options) {
        streamCalls += 1;
        if (streamCalls === 1) {
          yield { type: "thinking", delta: "stalling " };
        ...
        yield { type: "text", delta: "rescued and acting" };
        yield { type: "done", finishReason: "stop" };
      },
    };
    const pinned = buildPawNextTaskProfileV3({
      identity,
      profile: { ...streamProfile, configHash: first.configHash },
      apiKey: API_KEY,
    });
    const resolution = Object.freeze({
      ...pinned,
      taskOptions: Object.freeze({ ...pinned.taskOptions, model: stallModel }),
    });
    const events: string[] = [];
    const result = await runFreshPawNextTaskV3({
      resolution,
      thinkingRecovery: {
        policyVersion: "test:rescue-telemetry:60ms",
        noActionMs: 60,
        maxRecoveries: 1,
      },
    ...
    expect(result.assistantText).toBe("rescued and acting");
  });
});
