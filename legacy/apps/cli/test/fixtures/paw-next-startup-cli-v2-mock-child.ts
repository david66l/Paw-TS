...
if (!scenario || !workspaceRoot) {
  throw new Error("V2 startup CLI mock arguments are missing");
}

const sourceRoot = path.resolve(import.meta.dir, "..", "..", "src", "paw-next");
const catalogUrl = pathToFileURL(
  path.join(sourceRoot, "product-profile-catalog.ts"),
).href;
const scannerUrl = pathToFileURL(path.join(sourceRoot, "startup-scan.ts")).href;
const cliUrl = pathToFileURL(path.join(sourceRoot, "startup-cli-v2.ts")).href;
const secret = "sk-v2-raw-secret";
let catalogFactoryCalls = 0;
let resolverCalls = 0;
let scanCalls = 0;
let sourceWasV2Only = false;

mock.module(catalogUrl, () => ({
  createPawNextProductProfileCatalogV1(options: unknown) {
    catalogFactoryCalls += 1;
    sourceWasV2Only =
      JSON.stringify(options) === JSON.stringify({ workspaceRoot, v2: {} });
    if (scenario === "configuration_throw") {
      throw new Error(`catalog failed ${secret}`);
    }
    return () => {
      resolverCalls += 1;
      return undefined;
...
mock.module(scannerUrl, () => ({
  async scanAndResumePawNextRunsWithCatalogV1(input: {
    ...
    if (scenario === "scan_throw") {
      throw new Error(`scanner failed ${secret}`);
    }
    if (scenario === "configuration_throw") {
    ...
    return {
      issues: [{ entryName: "authority-entry", reason: "foreign_entry" }],
      runs: [
        run("terminal"),
        run("blocked_pending"),
        run("blocked_unconsumed"),
...
const { runPawNextStartupCliV2 } = await import(cliUrl);
let timeoutCalls = 0;
let intervalCalls = 0;
...
try {
  const result = await runPawNextStartupCliV2(
    scenario === "invalid_args"
      ...
      : ["--startup-scan-v2", "--root", workspaceRoot],
  );
