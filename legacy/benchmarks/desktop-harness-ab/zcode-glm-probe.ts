...
// Run ZCode with the exact frozen Paw task. Credentials remain on the host.
...
const root = fs.mkdtempSync(path.join(os.tmpdir(), "paw-zcode-glm-"));
...
async function forward(id: string) {
  ...
  try {
    ...
    if (body.model?.toLowerCase() !== "glm-5.3-flash")
    ...
    if (
      ...
      throw new Error("ZCode reasoning configuration mismatch");
    if (route === "/v1/messages") {
      if (requests >= maxRequests) throw new Error("Request budget exhausted");
      requests++;
      if (body.tools?.length && ++mainRequests > maxTurns)
        throw new Error("Main request budget exhausted");
    }
    append("wire.jsonl", {
      type: "request",
      id,
    ...
    const response =
      process.env.PAW_ZCODE_RELAY_CHECK === "1"
        ? new Response(
            [
              {
                type: "message_start",
...
const name = `paw-zcode-bench-${randomUUID().slice(0, 8)}`;
const cliArgs = [
  "--prompt",
  goal,
  "--json",
  ...
  "yolo",
  "--disallowed-tools",
  "Agent,SendMessage,Skill,WebFetch,WebSearch,CronCreate,CronDelete,CronList,CronUpdate,EnterPlanMode,ExitPlanMode,AskUserQuestion,ReadSessionContext",
];
const zcodeRoot = "D:/zcode/resources/glm";
const zcodeBundleHash = createHash("sha256")
  .update(fs.readFileSync(path.join(zcodeRoot, "zcode.cjs")))
  .digest("hex");
const args = [
  "run",
  "--rm",
  ...
  "--mount",
  `type=bind,source=${path.resolve(import.meta.dir, "zcode-file-relay.mjs")},target=/runner.mjs,readonly`,
  "--mount",
  `type=bind,source=${zcodeRoot},target=/opt/zcode,readonly`,
  "--mount",
  `type=bind,source=${path.join(out, "zcode-state")},target=/zcode-state`,
  "-e",
  "HOME=/tmp/zcode-home",
  "-e",
  "ZCODE_MODEL_TELEMETRY_ENABLED=0",
  "-e",
  `PAW_BENCH_EFFORT=${effort}`,
  image.stdout.trim(),
  "node",
  "/runner.mjs",
  ...cliArgs,
];
fs.mkdirSync(path.join(out, "zcode-state"));
write("protocol.json", {
  zcodeBundleHash,
  comparisonNotes: [
    "Official desktop-bundled runtime in Linux container; no native GUI validation",
    `Same pinned Node base, ${wallMs} ms / ${maxRequests} calls / ${maxTurns} main requests; accounting units differ`,
    "Coding/progress tools retained; unrelated tools denied; delegation, plugins, skills, MCP and memory disabled for isolated single-agent task",
    `Provider effort=${effort}, thinking budget=32000, output=128000; raw requests validated`,
  ],
  transportCheckOnly: process.env.PAW_ZCODE_RELAY_CHECK === "1",
  startedAt,
  workspaceRoot: root,
  goal,
  model: "glm-5.3-flash",
  ...
  wallMs,
  maxRequests,
  maxMainRequests: maxTurns,
  imageId: image.stdout.trim(),
  zcodeVersion: "0.16.1",
  cliArgs,
  isolation: protocol.isolation,
  sourceHashes: Object.fromEntries(
    ["zcode-glm-probe.ts", "zcode-file-relay.mjs", "verify.mjs", "workflow-task.ts", "verify-workflow.mjs"].map((file) => [
      file,
      createHash("sha256")
...
console.log(
  JSON.stringify({ event: "zcode.start", output: out, workspaceRoot: root }),
);
fs.mkdirSync(path.join(out, "source-snapshot"));
for (const file of [
  "zcode-glm-probe.ts",
  "zcode-file-relay.mjs",
  "verify.mjs",
  "workflow-task.ts",
  "verify-workflow.mjs",
...
const fileWatch = setInterval(() => {
  for (const file of [
    ...
    if (observedFiles.has(file) || !fs.existsSync(target)) continue;
    observedFiles.add(file);
    firstProductWriteMs ??= Date.now() - startedAt;
    append("files.jsonl", {
      at: Date.now(),
...
console.log(
  JSON.stringify({ event: "zcode.settled", exitCode, timedOut, requests }),
);
