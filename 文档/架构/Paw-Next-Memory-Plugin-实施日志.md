...
- V3 全量达到 112/133=84.21%。剩余错误显示全局追加边界说明会修复部分 join/弃答题，但也会扰动无关计数题，因此没有晋级全局 prompt。V5 改为 query-bound 路由：只在问题明确出现 before、last week、到达时间、提交日期或教育阶段时附加对应边界语义；普通问题保持 V3 prompt 字节不变。
- V5 曾加入“`count_members` 的答案字符串以 0 开头就改成 insufficient”的后处理，并得到 115/133。独立架构复核指出该规则没有集合闭包证书，会误杀合法 0，字符串实现还存在误匹配小数的风险；因此即使它贡献 1 题，也不作为合格架构晋级。V6 完全删除该 override；未来只有在 `exhaustiveSetRequired + aperture 完整 + 成员无歧义` 的闭包证书成立后，才允许区分已证明的 0 与证据不足。
